...
def train(data):
	...
	op_act = grid.best_estimator_.activation
	op_alpha = grid.best_estimator_.alpha

	print('Optimal Params (MLP):', op_act, op_alpha)
	...
	print('FEATURE SELECTION METRICS (MLP):')
	for j in range(20, 80, 5):
		...
		sgd = MLPClassifier(activation=op_act, alpha=op_alpha)
		fitted = sgd.fit(X_train_iter, y_train_iter)
	...
	mlp = MLPClassifier(activation=op_act, alpha=op_alpha)
	fitted = mlp.fit(X_general, y_general)
	...
	print('MLP:', 'acc:', round(accuracy, 2), 'precisions_by_label', [round(x, 2) for x in precisions_by_label], 'precision_global', round(precision_global, 2), 'recalls_by_label', [round(x, 2) for x in recalls_by_label], 'recall_global', round(recall_global, 2), 'f1s_by_label', [round(x, 2) for x in f1s_by_label], 'f1_global', round(f1_global, 2))
