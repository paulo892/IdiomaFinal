...
def train(data):
	...
	op_alg = grid.best_estimator_.algorithm
	op_n = grid.best_estimator_.n_neighbors

	print('Optimal Params (NEAREST NEIGHBOR):', op_alg, op_n)
	...
	print('FEATURE SELECTION METRICS (NEAREST NEIGHBOR):')
	for j in range(20, 80, 5):
		...
		dec_tree = KNeighborsClassifier(algorithm=op_alg, n_neighbors=op_n)
	...
	k_neighbors = KNeighborsClassifier(algorithm=op_alg, n_neighbors=op_n)
	fitted = k_neighbors.fit(X_general, y_general)
	...
	print('NEAREST NEIGHBORS:', 'acc:', round(accuracy, 2), 'precisions_by_label', [round(x, 2) for x in precisions_by_label], 'precision_global', round(precision_global, 2), 'recalls_by_label', [round(x, 2) for x in recalls_by_label], 'recall_global', round(recall_global, 2), 'f1s_by_label', [round(x, 2) for x in f1s_by_label], 'f1_global', round(f1_global, 2))
