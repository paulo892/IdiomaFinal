...
def train(data):
	...
	op_base = grid.best_estimator_.base_estimator
	op_estims = grid.best_estimator_.n_estimators
	op_boot = grid.best_estimator_.bootstrap


	print('Optimal Params (BAGGING):', op_base, op_estims, op_boot)
	...
	print('FEATURE SELECTION METRICS (BAGGING):')
	for j in range(20, 80, 5):
		...
		bag = BaggingClassifier(base_estimator=op_base, n_estimators=op_estims, bootstrap=op_boot)
		fitted = bag.fit(X_train_iter, y_train_iter)
	...
	bag = BaggingClassifier(base_estimator=op_base, n_estimators=op_estims, bootstrap=op_boot)
	fitted = bag.fit(X_general, y_general)
	...
	print('BAG:', 'acc:', round(accuracy, 2), 'precisions_by_label', [round(x, 2) for x in precisions_by_label], 'precision_global', round(precision_global, 2), 'recalls_by_label', [round(x, 2) for x in recalls_by_label], 'recall_global', round(recall_global, 2), 'f1s_by_label', [round(x, 2) for x in f1s_by_label], 'f1_global', round(f1_global, 2))
